...
public class LikesActivity extends BaseActivity
    ...
    LikesAdapter adapter = new LikesAdapter(likesList, u);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new LikesAdapter(likesList, u);
        recyclerView.setAdapter(adapter);
        initLikes();
        if(likesList.size() == 0) {
            AlertDialog.Builder dialog = new AlertDialog.Builder(LikesActivity.this);
            dialog.setTitle("提示");
            dialog.setMessage("您还没有喜欢过任何东西呢");
    ...
    private void initLikes() {
            ...
            String flag = null;
            String newsid = null;
            Cursor cursor = db.query("Likes", null, "username=?", new String[] {u}, null, null, "flag");
            if(cursor.moveToFirst()) {
                do {
                    ...
                    Likes likes = new Likes(image, name, description, url, flag, newsid);
                    likesList.add(likes);
                } while (cursor.moveToNext());
            }
            cursor.close();
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_alter) {
            Intent intent = new Intent(LikesActivity.this, AlterActivity.class);
            intent.putExtra("extra_data", u);
            intent.putExtra("status", 7);
            startActivity(intent);
        } else if (id == R.id.nav_hot) {
            Intent intent = new Intent(LikesActivity.this, VitalActivity.class);
            intent.putExtra("extra_data", u);
            startActivity(intent);
            ...
            Intent intent = new Intent(LikesActivity.this, CollectionActivity.class);
            intent.putExtra("extra_data", u);
            startActivity(intent);
        } else if (id == R.id.nav_like) {
            Toast.makeText(LikesActivity.this, "您当前已在我的喜欢页面", Toast.LENGTH_SHORT).show();
        }
        DrawerLayout drawer = (DrawerLayout) findViewById(R.id.drawer_layout);
        drawer.closeDrawer(GravityCompat.START);
        return true;
